...
export function ArticleSearch({ initialQuery, onPick }: { initialQuery: string; onPick: (article: ArticleMatch) => void }) {
  ...
  return (
    <div className="article-search-box">
      ...
      {articles.length ? <div className="article-results">{articles.map((article) => <button key={article.variationId} onClick={() => { onPick(article); setQuery(article.title || article.variationName || query); setArticles([]); setSearched(false); }} type="button"><strong>{article.title || 'Kein Artikeltitel hinterlegt'}</strong><span className="article-result-ids"><span>Artikel-ID <b>{article.itemId}</b></span><span>Varianten-ID <b>{article.variationId}</b></span></span><small><b>Variante:</b> {article.variationName || '–'}</small><small><b>Modell:</b> {article.model || '–'}</small></button>)}</div> : null}
    </div>
  );
}
