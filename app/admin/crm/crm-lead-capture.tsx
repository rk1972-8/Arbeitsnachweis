...
export function CrmLeadCapture({ onExtract }: { onExtract: (result: CrmContactExtraction) => void }) {
  ...
  async function analyze() {
    ...
    try {
      const response = await fetch('/api/admin/crm/extract', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ text, imageDataUrl: photo, mode }) });
  ...
  return <section className="crm-capture-card">
    <div className="crm-capture-head"><div><span className="card-kicker">Schnellerfassung</span><h3>Diktieren, Text einfügen oder fotografieren</h3><p>Die erkannten Angaben werden nur in die Felder übernommen. Vor dem Speichern kannst du alles kontrollieren.</p></div><span className="ai-badge">KI-Auswertung</span></div>
